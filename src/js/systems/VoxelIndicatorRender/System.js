...
export class VoxelIndicatorRenderSystem extends System {
    constructor(scene, camera) {
        ...
        console.log('🧊 VoxelIndicatorRenderSystem created with:', 
    ...
    createVoxelIndicatorGroup(entity, voxelComponent) {
        ...
        for (let i = 0; i < voxelComponent.voxelCount; i++) {
            ...
            const worldPos = voxelComponent.getVoxelWorldPosition(coords.x, coords.y, coords.z);
    ...
    addEntity(entity) {
        ...
        if (this.voxelIndicators.has(entity.id)) {
            console.log(`⚠️ VoxelIndicator already exists for entity ${entity.id}, skipping`);
            return;
        }
        
        const indicatorData = this.createVoxelIndicatorGroup(entity, voxelComponent);
        this.voxelIndicators.set(entity.id, indicatorData);
        this.scene.add(indicatorData.group);
        
        console.log(`🧊 Added voxel indicator for entity ${entity.id} (${voxelComponent.voxelCount} voxels)`);
    }
    ...
    updateVoxelIndicator(entity, indicatorData, deltaTime) {
        ...
        cubes.forEach((cube, index) => {
            ...
            if (material.uniforms) {
                ...
                // Update brightness
                material.uniforms.brightness.value = voxel.brightness * component.brightness;
                ...
                if (voxel.animationType === 'appear') {
                    ...
                    if (index === 0) { // Debug first cube only
                ...
                const newOpacity = this.lerp(currentOpacity, targetOpacity * 0.8, lerpFactor); // 0.8 max opacity
        ...
        if (hasUpdates && Math.random() < 0.1) { // Debug some updates
            console.log('🧊 Component updated, visible voxels:', component.voxels.filter(v => v.visible).length);
        }
    }
    ...
    update(deltaTime) {
        ...
        if (Math.random() < 0.01) { // 1% chance per frame
            console.log('🧊 VoxelRenderSystem update, indicators:', this.voxelIndicators.size);
        }
        ...
        for (const [entityId, indicatorData] of this.voxelIndicators) {
            const entity = indicatorData.group.userData.entity || 
                          Array.from(this.world?.entities?.values() || []).find(e => e.id === entityId);
            
            // Check if entity still exists and has required components
            if (!entity || !entity.hasComponent(VoxelIndicatorComponent) || !entity.hasComponent(TransformComponent)) {
                console.log('🧊 Entity', entityId, 'missing components:', 
                           'entity exists:', !!entity, 
                           'has VoxelIndicator:', entity?.hasComponent(VoxelIndicatorComponent),
                           'has Transform:', entity?.hasComponent(TransformComponent));
