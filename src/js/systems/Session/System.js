  ...
  async findOrCreateSession(entity1, entity2) {
    ...
    if (persistenceSystem && persistenceSystem.initialized) {
      try {
        ...
        console.log(
          `🔍 Checking ${allSessions.length} existing sessions for reuse...`,
        );
        ...
        for (const sessionData of sortedSessions) {
          ...
          if (
            sessionTitle.toLowerCase().includes(entity1Tag.toLowerCase()) &&
            ...
            if (sessionData.messageCount === 0) {
              console.log(
                `♻️ Found empty session to reuse: ${sessionData.title}`,
              );
              ...
              sessionComp1.activeSessions.set(
                restoredSession.id,
                restoredSession,
              );
              sessionComp2.activeSessions.set(
                restoredSession.id,
                restoredSession,
              );
        ...
        for (const sessionData of sortedSessions) {
          if (sessionData.messageCount === 0) {
            console.log(
              `♻️ Found any empty session to reuse: ${sessionData.title || sessionData.id}`,
            );
    ...
    for (const session of existingSessions) {
      ...
      const chatLog =
        entity1.getComponent(ChatLog) || entity2.getComponent(ChatLog);
      if (chatLog) {
  ...
  getSessionsBetweenEntities(entity1, entity2) {
    ...
    if (sessionComp1) {
      for (const [sessionId, sessionData] of sessionComp1.activeSessions) {
        if (
          sessionData.participants.has(entity1.id) &&
