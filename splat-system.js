...
class SplatSystem {
  ...
  createGaussianSplatField() {
    ...
    // Define cluster centers with proper syntax
    this.clusters = [
      {
        center: { x: 0, y: 0, z: 0 },
    ...
    for (let i = 0; i < splatCount; i++) {
      ...
      const cluster =
        this.clusters[Math.floor(Math.random() * this.clusters.length)];
  ...
  update(time, dataMetrics) {
    ...
    for (let i = 0; i < sizes.length; i++) {
      const dynamicScale =
        1 + Math.sin(time * 0.5 + i * 0.1) * 0.3 * dataMetrics.complexity;
...
window.SplatSystem = SplatSystem;
